...
**The destination is pinned at full resolution; only the route is withheld.** The line between the two is drawn at the boundary, not at how specific the wording is. Everything the requester observes when the work lands - an outcome, and the exact shape of an outcome they required: an error string, a response format, a rendered layout - is the destination, pinned as precisely as it needs to be. The mechanism the ticket leaves open is the interior route two correct builds could reach different ways. A precisely specified output is the destination at full resolution, not a mechanism to strip - an earlier framing keyed the distinction on specificity and made agents withhold legitimate outputs, the exact failure this guards against. A constraint the requester imposed on the route ("must use tmux") is a boundary condition too and stays; a route the author invented does not.
