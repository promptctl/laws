...
My reading of the SKILL.md file (this is inference, not a stated goal): the skill is built to keep the craft out of the orchestrating session. SKILL.md dispatches the writing to a fresh subagent and tells the caller not to read `craft.md`, on the stated grounds that it is the writing agent's material and loading it where it isn't needed stacks guidance. The design choice matches the skill's own economy-of-attention goal, applied to the agent rather than the reader.
