...
- Persistent agent guidance — CLAUDE.md files, system prompts, skill bodies, hook text, standing instructions for an LLM — load Skill(laws:guidance).
...
Reminder: your quality bar comes from the medium of the current deliverable — code → Skill(laws:code), persistent agent guidance → Skill(laws:guidance), human prose → Skill(laws:prose). If the matching skill is not loaded yet, load it before continuing.
EOT
