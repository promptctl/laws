...
**A wrong comment is fixed by shrinking it.** Polishing is subtraction — a pass that grows the comment is patching, not fixing. So the rule has an edge you can check: **the replacement is shorter than what it replaced**, and often it is nothing at all, because the code beneath it already says the true thing. If your corrected comment is longer than the wrong one, you didn't fix it, you fed it.
