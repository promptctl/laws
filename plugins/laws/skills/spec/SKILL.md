...
The process uses three documents, each with a single job:
...
A standard traceability matrix connects all three, so every component traces back to a user request and anything that doesn't trace back gets cut. The terms and section names are industry standard, so a new team member can follow the documents without learning a new vocabulary.
...
- Traceability matrix and iteration cycle, spanning all three: `references/traceability-and-iteration.md`
