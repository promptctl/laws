...
| `go-template-heldout-conformance` | `promptctl/go-template-js` | reimplement seven gutted helpers whose coverage was removed | **held-out coverage**: pristine tests restored + a hidden Go-generated conformance corpus injected at check time |
...
is caught mechanically, while adding new tests stays legitimate.
`go-template-heldout-conformance` needs no detector at all: its criterion *restores* the entire
test surface from a fresh clone of the pinned commit and injects a held-out corpus
(`heldout-fixtures/`, expected outputs generated by the real Go implementation) that the agent
never saw. Held-out coverage is only as hidden as the reference it was generated from is
unreachable: the 2026-08-01 campaign showed agents rebuilding an equivalent oracle from the
in-checkout Go generator and iterating to byte-perfection (see `../suites/code/SENSITIVITY.md`),
so the headroom this task measures today is narrower than its corpus suggests. Every injected defect was
